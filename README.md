...
用户标注属于长期知识，会随配置文档同步；多设备合并按标注 ID 取并集，同一标注按 `updatedAt` 与设备 ID 决定最后写入。普通 Jev 缓存、语义 token 和临时运行状态只保存在本机 IndexedDB，不上传 S3，也不参与实时过滤请求。

Bucket 需要允许扩展来源执行 GET、PUT 和 CORS 预检。

### Local decisions and feedback

Background Worker 按“用户标注 → 精确缓存 → 归一化缓存 → 模板缓存 → 语义缓存 → Jev”处理内容。缓存绑定当前页面策略与 Provider 的稳定指纹；策略内容、顺序、敏感度或 Provider 改变后，旧缓存不会跨版本复用。

每条完成判定的内容右上角都有 `J` 入口，可查看命中率和来源，并选择：
...
前两项只作用于当前内容；后两项会作为本地语义样本。显式用户选择始终高于缓存和 Jev。普通缓存默认保留 7 天，并在后台按最近访问时间清理，总量最多保留 5,000 条；模板缓存至少需要两个一致样本，自动语义复用则要求多个高相似且结论一致的样本。
