...
export function StrategyPreview({
  ...
  return (
    <aside
      className="min-w-0 rounded-[15px] border border-line-strong bg-surface/60 p-[18px] min-[961px]:sticky min-[961px]:top-6 min-[1151px]:p-[22px]"
      aria-label="策略预览"
    >
      <div className="mb-[22px] flex items-start justify-between gap-2.5">
        <div>
          <span className={eyebrow}>LIVE PREVIEW</span>
          <h2 className="mt-[7px] text-base font-semibold tracking-[-0.02em]">看看它如何呈现</h2>
        </div>
        <span className="pt-0.5 text-[9px] whitespace-nowrap text-muted before:mr-[5px] before:inline-block before:size-[5px] before:rounded-full before:bg-signal before:content-['']">
          本地模拟
        </span>
      </div>
      <div className="mb-2.5 flex items-center justify-between gap-3 text-[11px] text-muted">
        <span>{surface === "timeline" ? "时间线博文" : "评论区"}</span>
        <select
          className={`${control} min-h-0 w-auto px-2 py-1 text-[11px]`}
          aria-label="预览主题"
          value={theme}
      ...
      <div
        className={cn(
          "overflow-hidden rounded-xl border border-[#d2dfe1] bg-white text-[#0f1419]",
          theme === "dim" && "border-[#38444d] bg-[#15202b] text-[#f7f9f9]",
          theme === "dark" && "border-[#333639] bg-black text-[#e7e9ea]",
        )}
      >
        <article ref={articleRef} className="min-h-[290px] bg-[inherit]">
          <div className="p-[18px]">
            <div className="flex items-center gap-2.5 text-xs">
              <span className="grid size-[34px] place-items-center rounded-full bg-[#d9eeeb] font-semibold text-[#275e63]">
                L
              </span>
              <div className="grid">
                <strong className="text-xs">Lin / 林</strong>
                <span className="text-[10px] opacity-65">@lin_notes · 刚刚</span>
              </div>
              <span className="ml-auto text-xl">···</span>
            </div>
            <p className="my-[15px] text-xs break-words whitespace-pre-wrap">{text}</p>
            <div className="relative flex h-[100px] items-center justify-between overflow-hidden rounded-lg bg-[#dbe9e7] p-5 text-[#376467]">
              <span className="font-mono text-base leading-[1.4] font-bold tracking-[-0.06em]">
                LESS NOISE.
                <br />
                MORE SIGNAL.
              </span>
              <i
                className="size-[70px] rounded-full border border-[#91b6b2] shadow-[0_0_0_16px_#dbe9e7,0_0_0_17px_#a9c7c3]"
                aria-hidden="true"
              />
            </div>
            <div className="mt-4 flex justify-between font-mono text-[10px] opacity-60">
              <span>♡ 24</span>
              <span>↻ 8</span>
              <span>↗ 128</span>
            </div>
      ...
      <div className="flex min-h-[46px] flex-wrap items-center justify-between gap-2.5 text-[10px] text-signal">
        <span>{!hit ? "未达到阈值 · 保持可见" : revealed ? "已揭示 · 内容可见" : "已命中 · Hover 查看变量"}</span>
        <button
          className="bg-transparent px-0 py-[5px] text-[10px] text-signal disabled:opacity-50"
          type="button"
      ...
      <label className={`${field} mt-2 border-t border-line pt-4`} htmlFor="preview-rate">
        ...
        <input
          className="h-6 w-full cursor-pointer accent-signal"
          id="preview-rate"
          type="range"
          min="0"
          max="100"
      ...
      <p className="mt-4 text-[10px] leading-[1.8] text-muted">
