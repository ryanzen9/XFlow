...
async function run(page) {
  ...
  await page.reload();
  await page.setViewportSize({ width: 388, height: 650 });
  ...
  return { result: "PASS", checks: ["provider status", "no key field", "switch state", "layout"], errors };
}
